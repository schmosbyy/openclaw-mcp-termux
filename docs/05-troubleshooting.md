...
| `GATEWAY_AUTH_FAILED` | Wrong OpenClaw Gateway Token | Check `grep OPENCLAW_GATEWAY_TOKEN ~/.openclaw/.env` and ensure it matches the `OPENCLAW_GATEWAY_TOKEN` inside `.env`. |
| `GATEWAY_UNREACHABLE` | Gateway not running | Ensure `openclaw-proot.sh gateway` has been executed on the phone. |
| Gateway not starting | proot-Ubuntu not installed | Run `proot-distro install ubuntu` then retry `openclaw-proot.sh gateway` |
